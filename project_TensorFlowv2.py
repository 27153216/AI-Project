...
pygame.display.set_caption(u'AI打殭屍(TensorFlowv2)')
...
def movewindow():
    ...
    window = win32gui.FindWindow(0, u'AI打殭屍(TensorFlowv2)')
    #不加這兩行會出錯
...
def show_train_rate():    #顯示訓練率
    ...
    plt.subplot(212)
    plt.title('loss_rate')
    plt.plot(loss_arr, label='x', color='blue')
    plt.grid()
    plt.pause(0.001)
    movewindow()
...
def normalize():    #標準化
    global runtick_array, zombie_pos_array, zombie_pos_array_max, zombie_pos_array_max0, zombie_pos_array_max1, zombie_pos_array_min0, zombie_pos_array_min1
    zombie_pos_array_max0 = np.max(np.array(zombie_pos_array)[:,0])
    zombie_pos_array_max1 = np.max(np.array(zombie_pos_array)[:,1])
    zombie_pos_array_min0 = np.min(np.array(zombie_pos_array)[:,0])
    zombie_pos_array_min1 = np.min(np.array(zombie_pos_array)[:,1])
    for i in range(len(runtick_array)):
        runtick_array[i][0] = runtick_array[i][0] / 199
        zombie_pos_array[i][0] = (zombie_pos_array[i][0]-zombie_pos_array_min0) / (zombie_pos_array_max0-zombie_pos_array_min0)
        zombie_pos_array[i][1] = (zombie_pos_array[i][1]-zombie_pos_array_min1) / (zombie_pos_array_max1-zombie_pos_array_min1)

def nextI():
    global model, model1, shoot_pos_array, runtick_array, zombie_pos_array,  loss_arr, loss_arr1
    #訓練
    history = model.fit(runtick_array, zombie_pos_array, batch_size=1, epochs=1, shuffle=True, verbose=1)
    loss_arr.append(history.history['loss'][0])
    shoot_pos_array = model.predict(runtick_array)  #根據runtick預測射擊點

    #去標準化
    for i in range(len(runtick_array)):
        shoot_pos_array[i][0] = shoot_pos_array[i][0] * (zombie_pos_array_max0-zombie_pos_array_min0) + zombie_pos_array_min0
        shoot_pos_array[i][1] = shoot_pos_array[i][1] * (zombie_pos_array_max1-zombie_pos_array_min1) + zombie_pos_array_min1
#    print(shoot_pos_array)
...
def game_loop():
    ...
    while gaming:
        ...
        if firstround:  #第一回合紀錄殭屍位置及runtick
            zombie_pos_array.append([zombie_pos[0], zombie_pos[1]])
            runtick_array.append([runtick])
        
        if runtick > 24 and runtick % 12 == 0 and ammo > 0 and not firstround:  #每0.2秒自動射擊
            shoot_pos = [int(shoot_pos_array[runtick][0]), int(shoot_pos_array[runtick][1])]
