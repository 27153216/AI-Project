...
pygame.display.set_caption(u'AI打殭屍(PSO)')
...
def movewindow():
    ...
    window = win32gui.FindWindow(0, u'AI打殭屍(PSO)')
    #不加這兩行會出錯
...
class Particle_Class:
    ...
    def get_fit(self, hit):
        self.fit = hit #該是否命中就是該次particle_n的合適度
...
#        if particle_n >= particle_num-1: #所有particle跑完，下一個Generation後，重置相關參數
        ...
        if best_score == 0: #如果一槍都沒中，particle重新random
            for i in range(particle_num):
                for j in range(particle_dim):
                    PSO.particle[i].x[j] = np.random.randint(display_width)
                    PSO.particle[i].x[j] = np.random.randint(display_height)
...
#        canvas.fill(white)  #顯示白畫面
        ...
        showFont(u'Iteration: '+str(generation)+', Particle: '+str(particle_n)+u', 按a加速, 按d恢復, 按z更新訓練率:'+str(auto_show_train_rate) ,10,10) #顯示世代
...
#Game Loop
def game_loop():
    global gaming, score, canvas, ammo, tzombie_moving, runtick, crosshair_pos, fps, PSO, particle_n, auto_show_train_rate, shoot
    while gaming:
        shoot = False
        ...
        if runtick > 24 and runtick % 12 == 0 and ammo > 0:  #每0.2秒自動射擊
            shoot_times = 10 - ammo
            particle_n = shoot_times #每開一槍particle_n+1，從零起算
#            print(PSO.particle[shoot_times].x)
            crosshair_pos = np.copy([i-crosshair_size/2 for i in PSO.particle[shoot_times].x])    #準星位置
            bhole_array.append([i-bhole_size/2 for i in PSO.particle[shoot_times].x])  #紀錄彈孔位置
            shoot = True
            shot.play()
            ammo -= 1
            #命中判定
            hit = 0 #該次particle未命中
            if zombie_pos[0]+zombie_size[0]/3 < PSO.particle[shoot_times].x[0] < zombie_pos[0]+zombie_size[0]/3*2 and zombie_pos[1] < PSO.particle[shoot_times].x[1] < zombie_pos[1] + zombie_size[1]:
                if np.random.rand() < 0.5: zombie_pain0.play()
                else: zombie_pain1.play()
                blood_array.append([i-blood_size/2 for i in PSO.particle[shoot_times].x])  #紀錄血跡位置
                score += 1
                hit = 1 #該次particle有命中
            PSO.particle[shoot_times].get_fit(hit)  #更新該particle的fit
