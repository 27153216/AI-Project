import pygame, threading, time, numpy as np, win32gui, win32com.client, win32api, win32con
...
pygame.display.set_caption(u'打殭屍')
clock = pygame.time.Clock()
pygame.mouse.set_visible(0) #隱藏滑鼠
gaming = True
fps = 60
score = 0
best_score = 0
...
def movewindow():
    ...
    window = win32gui.FindWindow(0, u'打殭屍')
    #不加這兩行會出錯
...
shot.set_volume(0.05)    #0~1
reload = pygame.mixer.Sound('sound/reload.wav')
reload.set_volume(0.1)
bgm = pygame.mixer.music.load('sound/Opera.mp3')
...
zombie_pain0.set_volume(0.1)
zombie_pain1 = pygame.mixer.Sound('sound/zombie_pain1.wav')
zombie_pain1.set_volume(0.1)
zombie_pos = [800,200]
zombie_n = zombie[0]
...
zombie_moving_fps = pygame.time.Clock()
def zombie_moving():    #殭屍移動
    global zombie_pos, score, ammo, best_score
    move_y = 5
    while gaming:
        zombie_pos[0] -= 5
        zombie_pos[1] += move_y
        ...
        if zombie_pos[0]<-200:  #跑到底了
            ...
            ammo = 10
            reload.play()
        zombie_moving_fps.tick(fps)
tzombie_moving = threading.Thread(target=zombie_moving)
tzombie_moving.start()

#Game Loop
def game_loop():
    global gaming, score, canvas, ammo, tzombie_moving, bhole_array, blood_array
    while gaming:
        shoot = False
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r:
                    blood_array = []
                    bhole_array = []
            elif event.type == pygame.MOUSEMOTION:  #滑鼠移動
                crosshair_pos = [i-crosshair_size/2 for i in pygame.mouse.get_pos()]
            elif event.type == pygame.MOUSEBUTTONDOWN and ammo > 0:  #滑鼠按下
                bhole_array.append([i-bhole_size/2 for i in pygame.mouse.get_pos()])  #紀錄彈孔位置
                shoot = True
                shot.play()
                ammo -= 1
                #命中判定
                if zombie_pos[0]+zombie_size[0]/3 < pygame.mouse.get_pos()[0] < zombie_pos[0]+zombie_size[0]/3*2 and zombie_pos[1] < pygame.mouse.get_pos()[1] < zombie_pos[1] + zombie_size[1]:
                    if np.random.rand() < 0.5: zombie_pain0.play()
                    else: zombie_pain1.play()
                    blood_array.append([i-blood_size/2 for i in pygame.mouse.get_pos()])  #紀錄血跡位置
                    score += 1
                
        #顯示區，越後面越上層
#        canvas.fill(white)  #顯示白畫面
        canvas.blit(bg, bg_pos)
        showFont(u'得分：'+str(score)+u'，目前最高分：'+str(best_score),300,550)
        showFont(u'按r刷新背景',10,10)
